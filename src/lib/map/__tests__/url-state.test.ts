...
describe("map url state", () => {
  it("round-trips filters and bbox", () => {
    const params = new URLSearchParams(
      "state=Maharashtra&risk=Green&bbox=72.8,18.4,73.0,18.6&listing=abc",
    );
    const parsed = parseMapUrlState(params);
    expect(parsed.filters.state).toBe("Maharashtra");
    ...
    expect(serialized).toContain("state=Maharashtra");
